// Test script to verify Why Moksha Seva integration
const fetch = require('node-fetch');

async function testWhyMokshaSevaIntegration() {
  console.log('🧪 Testing Why Moksha Seva Integration...\n');
  
  // Test 1: API Endpoint
  console.log('1. Testing Why Moksha Seva API...');
  try {
    const response = await fetch('http://localhost:5000/api/page-config/why-moksha-seva');
    const data = await response.json();
    if (data.success) {
      console.log('✅ Why Moksha Seva API working');
      ...
      console.log('❌ Why Moksha Seva API failed:', data.message);
    }
  } catch (error) {
    console.log('❌ Why Moksha Seva API error:', error.message);
  }
  ...
  console.log('\n🎯 Integration Summary:');
  console.log('✅ Backend: Why Moksha Seva config seeded');
  ...
  console.log('4. ✅ Why Moksha Seva - Dynamic & Admin Ready');
  ...
  console.log('3. Edit Why Moksha Seva config via admin panel');
